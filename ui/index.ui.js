function Screen(ctx) {
  ...
  // ===== 宿主侧直连本机网易云服务（localhost:3001，NCM后端+nginx反代）=====
  // fetch / XMLHttpRequest 双兜底，宿主没有 http 模块也能用
  function __http(url, method, bodyObj) {
        ...
        .catch(function (e) {
          ...
          if (typeof XMLHttpRequest !== "undefined") {
            return new Promise(function (res, rej) {
              ...
              x.onerror = function () { rej(new Error("xhr_fail")); };
            });
          }
          throw e;
        });
    ...
    return Promise.reject(new Error("no_network_api"));
  }

  function __ncm(path, method, bodyObj) {
    return __http("http://localhost:3001" + path, method, bodyObj)
      .catch(function (e) { return JSON.stringify({ error: "" + (e && e.message) + " | " + ("" + e) }); });
  }
  ...
  function __envProbe() {
    var info = {
      ...
      xhr: (typeof XMLHttpRequest !== "undefined") ? "yes" : "no"
    };
  ...
  panelController.addJavascriptInterface("NativeDreambox", {
    ...
    env_probe: function () {
      try { return __envProbe(); } catch (e) { return JSON.stringify({ error: "" + e }); }
    },
    music_search: async function (kw) {
      try {
        var raw = await __ncm("/search?keywords=" + encodeURIComponent(kw || "") + "&type=1&limit=10", "GET");
        return raw;
      } catch (e) {
        return JSON.stringify({ error: "" + (e && e.message) });
      }
    },
    music_url: async function (id, level) {
      try {
        var lv = level || "exhigh";
        var raw = await __ncm("/song/url?id=" + encodeURIComponent(id) + "&level=" + lv, "GET");
        var d = null;
        try { d = JSON.parse(raw); } catch (e2) { d = null; }
        if (!d || !d.data || !d.data[0] || !d.data[0].url) {
          raw = await __ncm("/song/url", "POST", { id: id, level: lv });
        }
        return raw;
      } catch (e) {
        return JSON.stringify({ error: "" + (e && e.message) });
  ...
  // ===== 网易云播放器逻辑 =====
  var musicListArr=[], curSong=null, audio=null;
  function fmtSec(s){ if(isNaN(s))return "00:00"; var m=Math.floor(s/60),sec=Math.floor(s%60); return ("0"+m).slice(-2)+":"+("0"+sec).slice(-2); }
  // 照抄天使核：WebView 直接 fetch 本机网易云服务（不走宿主桥接）
  var NCM = "http://127.0.0.1:3000";
  function ncmSearch(kw){
    return fetch(NCM + "/search?keywords=" + encodeURIComponent(kw||"") + "&limit=10")
      .then(function(r){ return r.json(); })
      .then(function(d){ return (d.result && d.result.songs) || []; })
      .catch(function(e){ throw new Error("search_fail:"+(e&&e.message)); });
  }
  function ncmUrl(id){
    return fetch(NCM + "/song/url?id=" + id)
      .then(function(r){ return r.json(); })
      .then(function(d){ return (d.data && d.data[0] && d.data[0].url) || ""; })
      .catch(function(e){ throw new Error("url_fail:"+(e&&e.message)); });
  }
  function doMusicSearch(){
    var kw=document.getElementById("musicKw").value.trim();
    var list=document.getElementById("musicList");
    list.innerHTML='<div class="note">搜歌中…</div>';
    ncmSearch(kw).then(function(songs){
      musicListArr=songs||[];
      if(!musicListArr.length){list.innerHTML='<div class="note">没搜到「'+esc(kw)+'」的歌</div>';return;}
      var html="";
      musicListArr.forEach(function(s,idx){
        var ar=(s.artists&&s.artists[0])?s.artists[0].name:"";
        html+='<div class="musItem" data-i="'+idx+'"><div class="mName">'+esc(s.name)+'</div><div class="mArtist">'+esc(ar)+' · '+fmtSec(s.duration/1000)+'</div></div>';
      });
      list.innerHTML=html;
      list.querySelectorAll(".musItem").forEach(function(el){
        el.onclick=function(){beep();playMusic(parseInt(el.getAttribute("data-i"),10));};
      });
    }).catch(function(e){list.innerHTML='<div class="note">搜歌失败（网易云服务可能没起，等自愈巡检拉起后重试）</div>';});
  }
  function playMusic(idx){
    var s=musicListArr[idx]; if(!s)return;
    curSong=s;
    document.getElementById("nowPlaying").textContent=s.name+" · "+((s.artists&&s.artists[0])?s.artists[0].name:"");
    document.querySelectorAll(".musItem").forEach(function(el){el.classList.remove("on");});
    var on=document.querySelector('.musItem[data-i="'+idx+'"]'); if(on)on.classList.add("on");
    ncmUrl(s.id).then(function(url){
      if(!url){toast("拿不到播放链接（可能无版权）");return;}
      audio=audio||document.getElementById("musicAudio");
      audio.src=url;
      var p=audio.play();
      if(p&&p.catch)p.catch(function(){toast("点了才能播（浏览器限制）");});
    }).catch(function(){toast("播放链接获取失败");});
  }
  function bindMusic(){
    var a=document.getElementById("musicAudio"); if(!a)return;
    a.addEventListener("loadedmetadata",function(){document.getElementById("progTot").textContent=fmtSec(a.duration);});
    a.addEventListener("timeupdate",function(){
      if(!a.duration)return;
      document.getElementById("progFill").style.width=((a.currentTime/a.duration)*100)+"%";
      document.getElementById("progCur").textContent=fmtSec(a.currentTime);
    });
    a.addEventListener("ended",function(){stepMusic(1);});
    function stepMusic(d){
      if(!musicListArr.length)return;
      var ci=musicListArr.findIndex(function(x){return x.id===curSong.id;});
      var ni=ci+d; if(ni<0)ni=musicListArr.length-1; if(ni>=musicListArr.length)ni=0;
      playMusic(ni);
    }
    document.getElementById("mPlay").onclick=function(){beep();
      if(!a.src){if(curSong)playMusic(musicListArr.findIndex(function(x){return x.id===curSong.id;}));return;}
      var p=a.play(); if(p&&p.catch)p.catch(function(){});
      document.getElementById("mPlay").textContent="❚❚";
    };
    document.getElementById("mPrev").onclick=function(){beep();stepMusic(-1);};
    document.getElementById("mNext").onclick=function(){beep();stepMusic(1);};
    document.getElementById("musicSearch").onclick=function(){beep();doMusicSearch();};
    document.getElementById("musicKw").addEventListener("keydown",function(e){if(e.key==="Enter"){beep();doMusicSearch();}});
  }

  // ===== ②「一起听」双人画面 =====
  function bindTogether(){
    // 头像换图（存 base64 到 state）
    function avImg(key, el){
      el.onclick=function(){beep();
        var input=document.createElement("input");input.type="file";input.accept="image/*";
        input.onchange=function(){
          var file=input.files[0];if(!file)return;
          var reader=new FileReader();
          reader.onload=function(e){
            var raw=e.target.result,img=new Image();
            img.onload=function(){
              var w=img.width,h=img.height,maxW=200;
              if(w>maxW||h>maxW){var s=maxW/Math.max(w,h);w=Math.round(w*s);h=Math.round(h*s);}
              var c=document.createElement("canvas");c.width=w;c.height=h;
              c.getContext("2d").drawImage(img,0,0,w,h);
              state[key]=c.toDataURL("image/jpeg",0.85);saveAll();
              el.style.backgroundImage="url("+state[key]+")";
            };
            img.src=raw;
          };
          reader.readAsDataURL(file);
    ...
    avImg("tgAvL",document.getElementById("tgAvL"));
    avImg("tgAvR",document.getElementById("tgAvR"));
    // 头像回填
    var al=document.getElementById("tgAvL"),ar=document.getElementById("tgAvR");
    if(state.tgAvL)al.style.backgroundImage="url("+state.tgAvL+")";
    if(state.tgAvR)ar.style.backgroundImage="url("+state.tgAvR+")";
    // 时间/距离手动定义（可点改，存 settings）
    function editable(el,key){
      el.onclick=function(){beep();
        var v=prompt((key==="tgTime"?"一起听时长":"相距"),el.textContent);
        if(v!=null&&v.trim()){el.textContent=v.trim();state.settings=state.settings||{};state.settings[key]=v.trim();saveAll();}
      };
      el.style.cursor="pointer";
      if(state.settings&&state.settings[key]){el.textContent=state.settings[key];}
    }
    editable(document.getElementById("tgDist"),"tgDist");
    editable(document.getElementById("tgTime"),"tgTime");
    // 点赞
    var lk=document.getElementById("tgLike");
    lk.onclick=function(){beep();
      lk.textContent=lk.textContent==="♡"?"❤":"♡";
      toast(lk.textContent==="❤"?"点了一颗心":"取消了一颗心");
    };
    // 一起听搜歌（共用 ncmSearch，结果进 tgList，点歌共用播放器）
    var tgk=document.getElementById("tgKw");
    function tgSearch(){
      var kw=tgk.value.trim();
      if(!kw){toast("想听啥？");return;}
      var list=document.getElementById("tgList");
      list.style.display="block";list.innerHTML='<div class="note">搜歌中…</div>';
      ncmSearch(kw).then(function(songs){
        musicListArr=songs||[];
        if(!musicListArr.length){list.innerHTML='<div class="note">没搜到「'+esc(kw)+'」</div>';return;}
        var html="";
        musicListArr.slice(0,8).forEach(function(s,idx){
          var ar2=(s.artists&&s.artists[0])?s.artists[0].name:"";
          html+='<div class="musItem" data-i="'+idx+'"><div class="mName">'+esc(s.name)+'</div><div class="mArtist">'+esc(ar2)+'</div></div>';
        });
        list.innerHTML=html;
        list.querySelectorAll(".musItem").forEach(function(el){
          el.onclick=function(){beep();playMusic(parseInt(el.getAttribute("data-i"),10));};
        });
      }).catch(function(){list.innerHTML='<div class="note">搜歌失败（音乐服务没起，最后再解决）</div>';});
    }
    tgk.addEventListener("keydown",function(e){if(e.key==="Enter"){beep();tgSearch();}});
  }
...
/* 网易云播放器 */
.musRow{display:flex;gap:8px;margin:10px 0;align-items:center}
.musRow input{flex:1;min-height:auto}
.musList{max-height:150px;overflow-y:auto;margin:6px 0}
.musItem{padding:9px 11px;font-size:12px;border-radius:12px;margin:6px 0;background:rgba(255,255,255,.35);cursor:pointer;line-height:1.5}
.musItem.on{background:var(--btn);color:#fff}
.musItem .mName{font-weight:600}
.musItem .mArtist{font-size:10px;opacity:.6}
.nowBox{margin-top:10px;padding:12px;border-radius:16px;background:rgba(255,255,255,.3);box-shadow:inset 2px 2px 6px var(--sh1),inset -2px -2px 6px var(--sh2)}
.npName{font-size:12px;font-weight:600;margin-bottom:8px;line-height:1.5}
.progWrap{display:flex;align-items:center;gap:8px}
.progT{font-size:10px;opacity:.6;font-family:"SF Pro Display",-apple-system,sans-serif;min-width:30px;text-align:center}
.progBar{flex:1;height:4px;background:rgba(255,255,255,.4);border-radius:4px;overflow:hidden}
.progFill{height:100%;width:0%;background:var(--btn);border-radius:4px}
.ctrlRow{display:flex;justify-content:center;align-items:center;gap:20px;margin-top:12px}
.ctrlBtn{font-size:18px;cursor:pointer;color:var(--ink);width:40px;height:40px;display:flex;align-items:center;justify-content:center;user-select:none}
.ctrlBtn.main{font-size:24px;width:52px;height:52px;border-radius:50%;background:var(--btn);color:#fff;box-shadow:5px 5px 12px var(--sh1),-5px -5px 12px var(--sh2)}
#musicAudio{display:none}
/* 一起听双人脸 */
.tgWrap{position:relative;margin:8px 0}
.tgWire{position:relative;width:100%;height:80px}
.tgWire svg{width:100%;height:100%}
.tgFlow{animation:tgDash 2s linear infinite}
@keyframes tgDash{to{stroke-dashoffset:-32}}
.tgAvatars{display:flex;justify-content:center;gap:60px;margin-top:-30px;position:relative;z-index:2}
.tgAv{width:54px;height:54px;border-radius:50%;background:radial-gradient(circle at 32% 30%,rgba(255,255,255,.8),rgba(255,255,255,.12) 45%),var(--btn);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:#fff;box-shadow:5px 5px 12px var(--sh1),-5px -5px 12px var(--sh2);background-size:cover;background-position:center}
.tgMeta{display:flex;justify-content:center;gap:14px;margin-top:14px;font-size:11px;opacity:.7}
.tgMeta b{font-family:"SF Pro Display",-apple-system,sans-serif;font-weight:500;opacity:1}
.tgRow{display:flex;gap:8px;margin-top:12px;align-items:center}
.tgRow input{flex:1;min-height:auto}
/* 今日来信弹层 */
...
<div class="wrap">
  ...
  <div class="page active" id="pInbox">
    ...
    <div class="glass">
      <h1>一起听</h1><div class="sub">双人同频 · 网易云</div>
      <div class="tgWrap">
        <div class="tgWire">
          <svg viewBox="0 0 180 80"><path d="M15,20 C60,90 120,90 165,20" fill="none" stroke="rgba(255,255,255,.15)" stroke-width="1.5"/><path d="M15,20 C60,90 120,90 165,20" fill="none" stroke="rgba(255,255,255,.6)" stroke-width="1.5" stroke-dasharray="4 12" class="tgFlow"/></svg>
        </div>
        <div class="tgAvatars">
          <div class="tgAv" id="tgAvL">你</div>
          <div class="tgAv" id="tgAvR">机</div>
        </div>
        <div class="tgMeta">
          <span>相距 <b id="tgDist">285 km</b></span>
          <span>一起听 <b id="tgTime">124 分钟</b></span>
        </div>
        <div class="tgRow">
          <input type="text" id="tgKw" placeholder="想听什么…">
          <span class="btn" id="tgLike" style="padding:8px 10px;font-size:14px">♡</span>
        </div>
      </div>
      <div id="tgList" class="musList" style="display:none"></div>
      <div class="glass" style="margin:12px 0 0">
      <h1 style="font-size:14px">搜歌播放</h1><div class="sub">基础播放器 · 进度/切歌</div>
      <div class="musRow">
        <input type="text" id="musicKw" placeholder="搜首歌…">
        <span class="btn" id="musicSearch" style="padding:8px 16px;font-size:12px;align-self:center">搜</span>
      </div>
      <div id="musicList" class="musList"><div class="note">搜首歌开始播放</div></div>
      <div class="nowBox">
        <div id="nowPlaying" class="npName">（还没选歌）</div>
        <div class="progWrap">
          <span id="progCur" class="progT">00:00</span>
          <div class="progBar"><div id="progFill" class="progFill"></div></div>
          <span id="progTot" class="progT">00:00</span>
        </div>
        <div class="ctrlRow">
          <span class="ctrlBtn" id="mPrev">⏮</span>
          <span class="ctrlBtn main" id="mPlay">▶</span>
          <span class="ctrlBtn" id="mNext">⏭</span>
        </div>
  ...
  <div class="page" id="pMe">
    ...
    <div class="glass">
      <h1>语音条</h1><div class="sub">动态波纹 · 跟时间流动（接TTS二期点亮，先把画面做出来）</div>
      <div class="vbar" id="vbar">
        <div class="vwave" id="vwave">
          <span></span><span></span><span></span><span></span><span></span>
          <span></span><span></span><span></span><span></span><span></span>
        </div>
        <div class="vprog"><div class="vprogFill" id="vprogFill"></div></div>
        <div class="vctrl">
          <span class="vbtn" id="vPlay">▶</span>
          <span class="vnote" id="vNote">还没语音 · 二期接TTS点亮</span>
        </div>
      </div>
      <div class="chips" style="margin-top:10px">
        <span class="btn ghost" id="vGen" style="padding:8px 14px;font-size:11px">生成今日语音条</span>
        <span class="note" style="margin:0;align-self:center">留个口子，二期接 Moss/sphere_voice 合成</span>
      </div>
    </div>
    <div class="glass">
